...
TilesDir = "/group/pawsey0160/waves/profound/tiles_0.3Res/" # specify location of saved detection segment files
DataDir = "/group/pawsey0160/waves/profound/detect_0.3Res/" # specify location of saved detection segment files
OutputDir='/group/pawsey0160/waves/scripts_Sabine/FixingOutput_Automatic/'
gaia=fread("/group/pawsey0160/waves/ref/GAIA/gaiastarmaskwaves.csv")
	
TargetCat=fread(commandArgs(trailingOnly=TRUE)[1])
RelevantFrames = unique(TargetCat$Frame)
print(RelevantFrames)
...
for(frame in RelevantFrames){
  ...
  if(file.exists(segimFilename_Input) & !file.exists(paste0(OutputDir, frame, '.zip'))){
    ...
    iimage=Rfits_read_image(paste0(TilesDir, 'i_', frame,'.fits'), ext=1)     
    Zimage=Rfits_read_image(paste0(TilesDir, 'Z_', frame,'.fits'), ext=1)
    imageRGB=list(R=Zimage$imDat, G=iimage$imDat, B=rimage$imDat)
    cat('images read in \n')
  
  
  
    SegimCat$segstats$starmask=SegimCat$segstats$xmax*0
    GroupCoordinates=Rwcs_p2s(SegimCat$segstats$xmax, SegimCat$segstats$ymax, header=rimage$hdr)
    SegimCat$segstats$RAmax=GroupCoordinates[,'RA']
    SegimCat$segstats$Decmax=GroupCoordinates[,'Dec']
  
    gamara=c(min(SegimCat$segstats$RAmax),max(SegimCat$segstats$RAmax))
    gamadec=c(min(SegimCat$segstats$Decmax),max(SegimCat$segstats$Decmax))
  
    ra=median(SegimCat$segstats$RAmax)
    dec=median(SegimCat$segstats$Decmax)
    gaia0=gaia[ra > gamara[1]-1.0 & ra < gamara[2]+1.0 & dec > gamadec[1]-1.0 & dec < gamadec[2]+1.0,]
    ...
    GroupCoordinates=Rwcs_p2s(SegimCat$groupstats$xmax, SegimCat$groupstats$ymax, header=rimage$hdr)
    ...
    frameBorder=850 # change this value
    ngroupThreshhold = 3
    magRange=c(9, 21.2)
    ...
    # if(!file.exists(paste0(OutputDir, 'waves_pro_r_detect_', frame, '.rds'))){
    #   pro_r_detect = profoundProFound(rimage, segim=SegimCat$segim, static_photom=TRUE, verbose=TRUE, box=100)
    #   saveRDS(pro_r_detect$segstats, paste0(OutputDir, 'waves_pro_r_detect_', frame, '.rds'))
    #   pro_Z_detect = profoundProFound(Zimage, segim=SegimCat$segim, static_photom=TRUE, verbose=TRUE, magzero=30, box=100)
    #   saveRDS(pro_Z_detect$segstats, paste0(OutputDir, 'waves_pro_Z_detect_', frame, '.rds'))
    #   segstats = cbind(SegimCat$segstats,col=pro_r_detect$segstats$mag-pro_Z_detect$segstats$mag)
    # }else{
    #   pro_r_detect_segstats=readRDS(paste0(OutputDir, 'waves_pro_r_detect_', frame, '.rds'))
    #   pro_Z_detect_segstats=readRDS(paste0(OutputDir, 'waves_pro_Z_detect_', frame, '.rds'))
    #   segstats = cbind(SegimCat$segstats,col=pro_r_detect_segstats$mag-pro_Z_detect_segstats$mag)
    # }
    ...
    #   if(SegimCat$groupstats$N100[Sel][ii]>100000){ # iterate to figure out what an appropriate value is here
    ...
    #     }
    #     Coordinates$x[ii] = sum(x_coords)/SegimCat$groupstats$N100[Sel][ii]
    #     Coordinates$y[ii] = sum(y_coords)/SegimCat$groupstats$N100[Sel][ii]
    #   }
    # }
  
  
    # box=ceiling(3*sqrt(SegimCat$groupstats$N100[Sel]))
    # box = box + (1 - (box%%2))
    # box[box<200]=200
    # box[box>999]=999
    # # add maximum box size of 100 here. 
    # # for big objects, make sure that the thumbnail is centered. 
    ...
    print(paste0(frame, ' output exists'))
  }
