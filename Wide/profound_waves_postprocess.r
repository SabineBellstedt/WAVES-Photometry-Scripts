...
PlotDir = paste0(PostprocessDir, "plots/")
...
ebv=fread(paste0(config$path$reference, config$referencefiles$dustmap))
gaia=fread(paste0(config$path$reference, config$referencefiles$gaiastars))
ghost=fread(paste0(config$path$reference, config$referencefiles$ghostmask))
AstroVerification = fread(paste0(config$path$reference, config$referencefiles$astrometricverification))
...
GhostmaskRadius = function(mag, BreakMag = 2.5, GhostBreakMag = 6){
	Intercept = 1.07
	Gradient = 0.09
	radius=10^(Intercept-Gradient*mag)
	radius=ifelse(mag>GhostBreakMag,10^(Intercept-Gradient*GhostBreakMag),radius)
	radius=ifelse(mag<BreakMag,10^(Intercept-Gradient*BreakMag),radius)
	return(radius)
}

StarmaskRadius = function(mag, BreakMag = 2.5){
	Intercept = 1.3
	Gradient = 0.13
	radius=10^(Intercept-Gradient*mag)
	radius=ifelse(mag<BreakMag,10^(Intercept-Gradient*BreakMag),radius)
...
foreach(j=(1:length(InputTargetCat$RA)))%dopar%{
	...
	PostprocesStub = paste0("waves_postprocessed_", ra, '_', dec, PostprocessVersionSuffix, '.parquet')
	PostprocessFilename = paste0(PostprocessDir,PostprocesStub)
	# if(file.exists(MeasureFilename) ){
	if(file.exists(MeasureFilename) & (!file.exists(PostprocessFilename))){
	# if(file.exists(paste0(MeasureDir,'waves_measured_', ra, '_', dec, '.rds'))  & (!file.info(paste0(PostprocessDir,"waves_postprocessed_", ra, '_', dec, '.rds'))$mtime > "2024-04-05 00:00:00 AWST")){
		...
		datafile0$FrameName=PostprocesStub
		...
		ghost0=ghost[ra > wavesra[1]-1.0 & ra < wavesra[2]+1.0 & dec > wavesdec[1]-1.0 & dec < wavesdec[2]+1.0,]
		# ghost_secondary0=ghost_secondary[ra > wavesra[1]-1.0 & ra < wavesra[2]+1.0 & dec > wavesdec[1]-1.0 & dec < wavesdec[2]+1.0,]
		...
		#
		extinc_coeffs = config$filtersettings$extinctioncoefficients
		filters = config$filtersettings$filters
		zeropoints = config$filtersettings$zeropoints
		#
		for(filter in filters){
			ext = as.numeric(extinc_coeffs[filters == filter])
			zp = as.numeric(zeropoints[filters == filter])
			#
			...
			#
			# first check if the output file has the column:
			if((paste0("flux_",filter,"t") %in% names(datafile0))){
				...
				message(paste0(filter, ' missing in measure output'))
				datafile0[[paste0("mag_",filter,"t")]]=NA
				datafile0[[paste0("flux_",filter,"t_uncorrected")]]=NA
				datafile0[[paste0("flux_",filter,"t")]]=NA
				datafile0[[paste0("flux_err_",filter,"t_uncorrected")]]=NA
				datafile0[[paste0("flux_err_",filter,"t")]]=NA
				datafile0[[paste0("mag_",filter,"c")]]=NA
				datafile0[[paste0("flux_",filter,"c")]]=NA
				datafile0[[paste0("flux_err_",filter,"c")]]=NA
			}
		...
		if(config$flag$crudcut){
			datafile0[skyRMS_mean > crudcut & noOPT_r<1 & noOPT_i<1,"class"]="artefact" # high sky RMS. Ideally we actually want a measure of the mean skyRMS that just ignores the r band when it's missing...	
		}
		...
		datafile0[is.na(mag_Zt)+is.na(mag_Yt)+is.na(mag_Jt)+is.na(mag_Ht)+is.na(mag_Kt) > is.na(flux_Zt)+is.na(flux_Yt)+is.na(flux_Jt)+is.na(flux_Ht)+is.na(flux_Kt)+2 & 
							noOPT_r < 1,"class"]="artefact" # bright/missing NIR, where r-band exists. This finds ghosted regions pretty well. 
		...
		# 					# noOPT_r < 1 & 
		# 					noIR_Z==1,"class"]="artefact" # bright/missing NIR, where r-band exists. This finds ghosted regions pretty well. 
		...
		# datafile0[(mag_rt-mag_Zt) < -0.75,"class"]="artefact" # improbable r-Z colour
		...
		datafile0=datafile0[is.na(flux_Zt), "mag_Zt_fake" := 1L]
		datafile0=datafile0[is.na(flux_Zt), "mag_Zt" := mag_Yt + alpha*(mag_it - mag_Yt) + beta]
		datafile0=datafile0[is.na(flux_Zt), "mag_Zc" := mag_Yc + alpha*(mag_ic - mag_Yc) + beta]
		datafile0=datafile0[is.na(flux_Zt), "mag_app_Zt" := mag_app_Yt + alpha*(mag_app_it - mag_app_Yt) + beta]
		
		alpha = -0.7044
		beta = 0.004
		sigma = 0.0194
		
		datafile0=datafile0[is.na(flux_Zt), "mag_Zt" := mag_it + alpha*(mag_rt - mag_it) + beta]
		datafile0=datafile0[is.na(flux_Zt), "mag_Zc" := mag_ic + alpha*(mag_rc - mag_ic) + beta]
		datafile0=datafile0[is.na(flux_Zt), "mag_app_Zt" := mag_app_it + alpha*(mag_app_rt - mag_app_it) + beta]


		# datafile0[(mag_rt-mag_Zt) < -0.75 & noIR_Z == 1 & noOPT_r < 1 & mag_Zt_fake == 1,"class"]="artefact" # improbable r-Z colour when using the masked Z-band data. 
		datafile0[(mag_rt-mag_Yt) < -0.75 & noIR_Y < 1 & noOPT_r < 1 & mag_Zt_fake == 1,"class"]="artefact" # improbable r-Y colour for cases where there's no Z
		# datafile0[(mag_it-mag_Yt) < -1 & mag_Zt_fake == 1,"class"]="artefact" # improbable i-Y colour for cases where there's no Z
		...
		BreakMag = 3.5
		radius = StarmaskRadius(mag = gaia0$phot_g_mean_mag, BreakMag = BreakMag)
		#
		...
		# now applying the ghostmask
		if(length(ghost0$mag)>0){
			maskedregion=coordmatch(coordcompare=datafile0[,list(RAmax,Decmax)],
									coordref=ghost0[,list(ra,dec)],
									rad=ghost0$radius,
									inunitref="deg",
									inunitcompare="deg",
									radunit="amin")
			maskedobjs=unique(maskedregion$ID[maskedregion$ID>0])
			datafile0[maskedobjs,"ghostmask"]=1
		}
		...
		# png(paste0(PlotDir,"waves_GAIAoffset_",ra,"_",dec,"_DR3_multipanel", PostprocessVersionSuffix, ".png"),width=45.0,height=30.0,units="cm",res=120)
		...
		filelist = list.files(config$referencefiles$maskingpolygons, full.names = TRUE)
		...
		maskedobjs=unique(maskedregion$ID[maskedregion$ID>0])
		# message(paste0('objects masked for NGC 1049: ', length(datafile0$RAmax[maskedobjs == TRUE]) ))
		...
		maskedobjs=unique(maskedregion$ID[maskedregion$ID>0])
		# message(paste0('objects masked for other GC: ', length(datafile0$RAmax[maskedobjs == TRUE]) ))
		datafile0[maskedobjs,"mask"]=1 # actively masking the region
		...
		maskedobjs=unique(maskedregion$ID[maskedregion$ID>0])
		# message(paste0('objects masked for star: ', length(datafile0$RAmax[maskedobjs == TRUE]) ))
		...
		#
		# automatically create the flux columns related to the relevant filters
	
		filter_columns = character()
		for (filter in filters) {
			filter_columns = c(filter_columns,
				paste0('flux_', filter, 't'),
				paste0('flux_err_', filter, 't'),
				paste0('flux_', filter, 't_uncorrected'),
				paste0('flux_err_', filter, 't_uncorrected'),
				paste0('flux_', filter, 'c'),
				paste0('flux_err_', filter, 'c'),
				paste0('mag_', filter, 't'),
				paste0('mag_', filter, 'c'))
		}
		summary_columns <- c("FrameName","FrameID","uberID","segID","xmax","ymax","censep",
		    ...
		    "groupID","Ngroup","mag_app_Zt",
		    "mag_app_gt","mag_app_rt","mag_app_it","mag_app_Yt",
		    filter_columns, 
		    "mag_Zt_fake", 
		    "mask","starmask","ghostmask","starscol","starssize","class",
		    "noOPT_r", "noOPT_i","noIR_Z", "noIR_Y")
		fdatafile0=datafile0[, ..summary_columns]
		...
		# #
		# saveRDS(trim,file=PostprocessFilename)
		write_parquet(fdatafile0, PostprocessFilename)
		#
		PlotFilename=paste0(PlotDir,"waves_objects_",ra,"_",dec, PostprocessVersionSuffix, ".png")
		print(PlotFilename)
		CairoPNG(PlotFilename,width=25.0,height=25.0,units="cm",res=240)
		#
		Rwcs_image(everything$pro_detect$image, keyvalues=everything$pro_detect$keyvalues)
		points(fdatafile0[flux_Zt > 10^(0.4*(8.9-WAVESdepth)) & starmask > 0,list(xmax,ymax)],pch=16,col="red",cex=0.25)
		# points(fdatafile0[ghostmask > 0,list(xmax,ymax)],pch=16,col="magenta",cex=0.25)
		# points(fdatafile0[CATAID > 0 & Z > 0.002 & NQ > 2 & starmask < 1,list(xmax,ymax)],pch=".",col="green",cex=0.25)
		points(fdatafile0[flux_Zt > 10^(0.4*(8.9-WAVESdepth)) & class=="artefact",list(xmax,ymax)],pch=4,col="cyan",cex=0.25)
		points(fdatafile0[class=="artefact",list(xmax,ymax)],pch=4,col="cyan",cex=0.25)
		points(fdatafile0[flux_Zt > 10^(0.4*(8.9-WAVESdepth)) &  class=="star",list(xmax,ymax)],pch=18,col="yellow",cex=0.5)
		points(fdatafile0[flux_Zt > 10^(0.4*(8.9-WAVESdepth))  & starmask < 1 & class=="ambiguous",list(xmax,ymax)],pch=1,col="blue",cex=0.75)
		points(fdatafile0[flux_Zt > 10^(0.4*(8.9-WAVESdepth))  & starmask < 1 & class=="galaxy",list(xmax,ymax)],pch=5,col="purple",cex=0.75)
		# now adding the points with the fake mag_Zt value if they're missing Z-band data
		points(fdatafile0[mag_Zt < WAVESdepth & starmask < 1 & mag_Zt_fake == 1 & class=="galaxy",list(xmax,ymax)],pch=5,col="magenta",cex=0.75)
		points(fdatafile0[mag_Zt < WAVESdepth  & starmask < 1 & noIR_Z == 1 & class=="ambiguous",list(xmax,ymax)],pch=1,col="lightblue",cex=0.75)
		points(fdatafile0[flux_Zt > 10^(0.4*(8.9-WAVESdepth)) & mask == 1,list(xmax,ymax)],pch=4,col="gray",cex=0.25)
		# points(fdatafile0[flux_Zt > 10^(0.4*(8.9-WAVESdepth)) &  ghostmask == 1 & starmask < 1 & class!="artefact",list(xmax,ymax)],pch=18,col="green",cex=0.4)
		...
		xy_ghost=Rwcs_s2p(RA=ghost0$ra,Dec=ghost0$dec,header=everything$pro_detect$header)
		#
		if(length(ghost0$ra)>0){
			for (i in 1:length(ghost0$ra)){
				x=xy_ghost[i,1]
				y=xy_ghost[i,2]
				r=ghost0$radius[i]*60/pixScale 
				draw.circle(x,y,r,border="aquamarine",lty=1,lwd=1)
			}
		}
		message("Z<21.1 gals: ",length(fdatafile0[flux_Zt > 10^(0.4*(8.9-WAVESdepth)) & starmask < 1 & class=="galaxy",segID]))
		...
		message("Star masked: ",length(fdatafile0[starmask > 0,segID]))
		message("noOPT_r: ",length(fdatafile0[noOPT_r > 0,segID]))
		message("noIR_Z: ",length(fdatafile0[noIR_Z > 0,segID]))
		title(main=paste0(ra," ",dec),
		      sub=paste0("Z<21.1 gals:",length(fdatafile0[flux_Zt > 10^(0.4*(8.9-WAVESdepth)) & starmask < 1 & class=="galaxy",segID]),
		                 ...
		                 " | Star masked: ",length(fdatafile0[starmask > 0,segID]),
		                 " | Masked: ",length(fdatafile0[mask > 0,segID]),
		...
		message(paste0(PostprocessDir, PostprocessFilename, ': output exists'))
	}
}
###
