...
PlotDir = paste0(DetectDir, "plots/")
...
foreach(j=(1:length(InputTargetCat$RA)))%dopar%{
  ...
  dec=as.character((format(round(InputTargetCat$Dec[j], 1), nsmall = 1)))
	...
	kids_r=Rfits_read_image(paste0(InputDir, 'VST/', config$version$tile, 'r_', ra, '_', dec, '_', config$version$tile, '.fits'), ext=1)
	kids_i=Rfits_read_image(paste0(InputDir, 'VST/', config$version$tile, 'i_', ra, '_', dec, '_', config$version$tile, '.fits'), ext=1)
	viking_Z=Rfits_read_image(paste0(InputDir, 'VISTA/', config$version$tile, 'Z_', ra, '_', dec, '_', config$version$tile, '.fits'), ext=1)
	viking_Y=Rfits_read_image(paste0(InputDir, 'VISTA/', config$version$tile, 'Y_', ra, '_', dec, '_', config$version$tile, '.fits'), ext=1)
	...
	# cat('---saving RDS---\n')
	# saveRDS(everything,file=paste0(DetectDir,'waves_detect_', ra, '_', dec, '.rds'))
	...
	cat('---saving FITS---\n')
	Rfits_write(everything, filename=paste0(DetectDir,'waves_detect_', ra, '_', dec, '.fits'), compress=TRUE, list_sub=c('segim', 'segim_orig', 'objects', 'objects_redo', 'sky', 
				'skyRMS', 'image', 'mask', 'segstats', 'groupstats', 'detect_mask_r', 'detect_mask_i', 'detect_mask_Z', 'detect_mask_Y'))
	
	Rfits_write_header(filename=paste0(DetectDir,'waves_detect_', ra, '_', dec, '.fits'), ext=1, 
